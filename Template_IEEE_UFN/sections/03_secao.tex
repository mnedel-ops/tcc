...
David Ausubel fundamentou a Aprendizagem Significativa \cite{Ausubel1963} no princípio de que novas ideias são assimiladas apenas quando ancoradas em conhecimentos prévios estáveis, processo denominado subsunção \cite{Espirito2020}. Para mitigar o distanciamento entre o que o estudante já sabe e o material novo, Ausubel \cite{Ausubel1963} propôs o uso de organizadores prévios, que são materiais introdutórios apresentados em um nível mais alto de abstração e generalidade. Esses materiais servem como pontes conceituais para destacar a importância do novo conteúdo \cite{Custodio2015}. No design de jogos, isso se manifesta em tutoriais que apresentam a estrutura do sistema antes da complexidade total, que permitem ao jogador a diferenciação progressiva do conhecimento \cite{Espirito2020, Custodio2015}.
...
\subsection{Creature Capture}
O gênero \textit{Creature Capture} define-se como uma subcategoria dos jogos de \textit{Role-Playing Games} (RPG) cujo ciclo principal de \textit{gameplay} é centrado no colecionismo, treinamento e combate tático mediado por criaturas sencientes.
Ao contrário dos RPGs tradicionais, no qual o jogador controla apenas um avatar, neste gênero o protagonista é um treinador que utiliza criaturas domadas como extensão das capacidades estratégicas do jogador no mundo virtual.
Uma característica técnica fundamental é a existência de um sistema de classificação detalhado, geralmente baseado em tipos ou espécies, que determina as forças, fraquezas e afinidades elementares de cada criatura, que exige do jogador o domínio de uma taxonomia complexa para obter sucesso \cite{sapach17}.

Os benefícios para o aprendizado ao utilizar este gênero são vastos e atacam diretamente a abstração de conteúdos científicos. O gênero promove a "tangibilização" do abstrato, ao transformar propriedades físicas invisíveis em atributos lúdicos tangíveis, e permite que o aluno manipule conceitos complexos por meio de representações simbólicas e imaginativas.
E pesquisas em neurociência validam o impacto cognitivo desse gênero.
Estudos demonstraram que a experiência prolongada com criaturas de jogos de captura organiza o córtex visual de forma única, permitem a individuação de centenas de exemplares visualmente similares, mas semanticamente distintos.
Essa descoberta sugere que a familiaridade com as criaturas-elemento pode reduzir a carga cognitiva estranha.
Ao invés de memorizar símbolos abstratos, o aluno reconhece as criaturas, o que facilita a retenção de longo prazo e a recuperação de informações científicas complexas por meio de representações antropomórficas e lúdicas \cite{Gomez2019,Plass2020}.
